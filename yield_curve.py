...
class yield_curve():
    ...
    def tbond(self):
        ...
        if self._tbond_df is None:
            ...
            year_dict = [(pd.Timestamp.now() - pd.Timedelta(days=365.25*n)).strftime('%Y') for n in range(0,10)]
            df = pd.DataFrame()
    ...
    def oat(self):
        ...
        if self._oat_df is None:
            ...
            url = 'https://webstat.banque-france.fr/fr/downloadFile.do?id=5385691&exportType=csv'
            response = requests.get(url, headers={'User-Agent': 'Safari/537.36'}, timeout= 10)
            csv_file = io.StringIO(response.text)
            df = pd.read_csv(csv_file, header=5, sep = ';')

            # Clean and process the data
            columns = ['Date','10 Yr', '1 Mo', '1 Yr', '2 Yr', '30 Yr','3 Mo','5 Yr','6 Mo','9 Mo',]
            df.columns = columns
            ...
            # Sort and fill missing values
            df = df.sort_index()
            df = df[['1 Mo', '3 Mo', '6 Mo', '9 Mo', '1 Yr', '2 Yr', '5 Yr', '10 Yr', '30 Yr']]
            df = df.resample('D').ffill()
            self._oat_df = df
        return self._oat_df
